...
class Perfiles_clinicosController:
    ...
    def get_by_usuario_id(self, user_id: int):
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM perfiles_clinicos WHERE id_usuario = %s", (user_id,))
            columns = [desc[0] for desc in cursor.description]
            row = cursor.fetchone()
            if not row:
                raise HTTPException(status_code=404, detail="Perfil clínico no encontrado para el usuario")
            return {"resultado": dict(zip(columns, row))}
        except psycopg2.Error as err:
            raise HTTPException(status_code=500, detail=str(err))
        finally:
